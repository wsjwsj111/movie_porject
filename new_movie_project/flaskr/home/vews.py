...
seachrview = []
@home.route("/newbase",methods=["GET", "POST"])
def newbase():
    if request.method == "POST":
        name = request.form.get('title', None)
        # print("1234567")
        print(name)
        seachrview.append(name)
        return redirect("/search",userview=seachrview)
...
def search():
    # if request.method == "POST":
    #     testname = request.form.get('testname', None)
    #     print(testname)
    #     # global testval
    #
    #     nameinfo = model.movie.query.filter_by(title = testname).first()
    #     # global newname
    #     # print(name)
    #     # testval=[]
    #     # return render_template("search.html", newname=nameinfo)
    #     return render_template("search.html",nameinfo = nameinfo)

    global seachrview
    print(seachrview)
    nameinfo = model.movie.query.filter_by(title=seachrview[0]).first()
    print(nameinfo)
    # seachrview = []
...
def playvideo():
    global seachrview
    print(seachrview)
    movieinfo = model.movie.query.filter_by(title=seachrview[0]).first()
    # print(movieinfo)
    return render_template("playvideo.html",movieinfo=movieinfo)
